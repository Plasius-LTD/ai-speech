...
describe("npm release trust boundary", () => {
  it("runs pull requests on hosted capacity and trusted main on approved runners", () => {
    expect(ciWorkflow).toContain("runs-on: ${{ fromJSON(github.event_name == 'pull_request' && '{\"labels\":[\"ubuntu-latest\"]}' || '{\"group\":\"Public CI - Quarantined\",\"labels\":[\"self-hosted\",\"Linux\",\"X64\"]}') }}");
